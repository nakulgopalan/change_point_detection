...
npz_file_handler = np.load('/home/ng/workspace/corl_2019_all_code/data/final_files/full_behavior/segment_run_movo.npz', allow_pickle=True)
...
path_right = '/home/ng/workspace/corl_2019_all_code/data/final_files/right/'
path_left = '/home/ng/workspace/corl_2019_all_code/data/final_files/left/'
path_straight = '/home/ng/workspace/corl_2019_all_code/data/final_files/full_behavior/'


#all paths! #[path_straight]
...
pickle_path = '/home/ng/workspace/corl_2019_all_code/data/final_files/movo_only_full_data.pkl'
...
if True:
        ...
        for count, skill in enumerate(skills_present):
            if skill ==skill_chosen:
                ...
                for _ in range(slice_of_states_needed.shape[0]):
                    ...
                    sentence_id_list.append(str(trajectory_of_interest)+"-" + str(count) + "-" + \
    ...
    full_array = full_array[:,410:680]
    # full_array = full_array[:, 100:-100]
    ...
    cluster_labels0 = DBSCAN(eps=88, min_samples=2).fit_predict(full_array)# , min_samples=2
    #
    # cluster_labels = DBSCAN(metric=euclidean_and_cosine_similarity, eps=30, min_samples=10).fit_predict(full_array)
    #
    #
    plt.close()
    print("cluster0: ", cluster_labels0)
    n = 0
    plt.scatter(X_pca[n:, 0], X_pca[n:, 1], c=cluster_labels0)
    plt.title("PCA")
    for i, txt in enumerate(sentence_id_list):
        plt.annotate(txt, (X_pca[i, 0], X_pca[i, 1]))
    plt.colorbar(ticks=range(10))
    ...
    # cluster_labels1 = DBSCAN(eps=80, min_samples=2).fit_predict(full_array)  # , min_samples=2
    # #
    # # cluster_labels = DBSCAN(metric=euclidean_and_cosine_similarity, eps=30, min_samples=10).fit_predict(full_array)
    # #
    # #
    # plt.close()
    # print("cluster1: ", cluster_labels1)
    # n = 0
    # plt.scatter(X_pca[n:, 0], X_pca[n:, 1], c=cluster_labels1)
    # plt.title("PCA")
    # for i, txt in enumerate(sentence_id_list):
    #     plt.annotate(txt, (X_pca[i, 0], X_pca[i, 1]))
    # plt.colorbar(ticks=range(10))
    ...
    # cluster_labels2 = DBSCAN(eps=0.1, min_samples=4).fit_predict(full_array)  # , min_samples=2
    # #
    # # cluster_labels = DBSCAN(metric=euclidean_and_cosine_similarity, eps=30, min_samples=10).fit_predict(full_array)
    # #
    # #
    # # plt.close()
    # print("cluster2: ", cluster_labels2)
    # n = 0
    # plt.scatter(X_pca[n:, 0], X_pca[n:, 1], c=cluster_labels2)
    # plt.title("PCA")
    # for i, txt in enumerate(sentence_id_list):
    #     plt.annotate(txt, (X_pca[i, 0], X_pca[i, 1]))
    # plt.colorbar(ticks=range(10))
    ...
    # cluster_labels3 = DBSCAN(eps=1, min_samples=4).fit_predict(full_array)  # , min_samples=2
    #
    # cluster_labels = DBSCAN(metric=euclidean_and_cosine_similarity, eps=30, min_samples=10).fit_predict(full_array)
    #
    #
    # plt.close()
    # print("cluster3: ", cluster_labels3)
    # n = 0
    # plt.scatter(X_pca[n:, 0], X_pca[n:, 1], c=cluster_labels3)
    # plt.title("PCA")
    # for i, txt in enumerate(sentence_id_list):
    #     plt.annotate(txt, (X_pca[i, 0], X_pca[i, 1]))
    # plt.colorbar(ticks=range(10))
    ...
    # cluster_labels4 = DBSCAN(eps=0.001, min_samples=3).fit_predict(full_array)  # , min_samples=2
    # #
    # # cluster_labels = DBSCAN(metric=euclidean_and_cosine_similarity, eps=30, min_samples=10).fit_predict(full_array)
    # #
    # #
    # plt.close()
    # print("cluster4: ", cluster_labels4)
    # n = 0
    # plt.scatter(X_pca[n:, 0], X_pca[n:, 1], c=cluster_labels4)
    # plt.title("PCA")
    # for i, txt in enumerate(sentence_id_list):
    #     plt.annotate(txt, (X_pca[i, 0], X_pca[i, 1]))
    # plt.colorbar(ticks=range(10))
    ...
    symbol_assignment_dict = {}
    cluster_choice = cluster_labels0

    # save symbols in assignment dict!
    for counter, id in enumerate(sentence_id_list):
        if id in symbol_assignment_dict:
            if cluster_choice[counter] in symbol_assignment_dict[id]:
                continue
            else:
                symbol_assignment_dict[id].append(cluster_choice[counter])
                print("id has multiple labels: ", id, ", ", cluster_choice[counter], ",  ", symbol_assignment_dict[id])
        else:
            symbol_assignment_dict[id] = [cluster_choice[counter]]


    dict_obj_to_save = {}
    dict_obj_to_save['symbol_assignment'] = symbol_assignment_dict
    dict_obj_to_save['sentence_id_list'] = sentence_id_list
    dict_obj_to_save['clusters'] = cluster_choice
    dict_obj_to_save['states'] = full_array

    # pickle_path_symb_zero = '/media/ng/7ccf8f98-7ab8-498b-b405-54df784c3191/ng/workspace/bayesian_changepoint_detection/bnpy/examples/08_mocap6/skill-1-symbol_data_without_firealarm.pickle'
    pickle_path_symb_zero = '/home/ng/workspace/corl_2019_all_code/data/symb_skill0.pkl'

    with open(pickle_path_symb_zero, "wb") as f:
        pickle.dump(dict_obj_to_save, f)
    #
...
#     dict_obj_to_save['states'] = full_array
#
#     pickle_path_symb_zero = '/media/ng/7ccf8f98-7ab8-498b-b405-54df784c3191/ng/workspace/bayesian_changepoint_detection/bnpy/examples/08_mocap6/skill-0-symbol_data_without_firealarm_88.pickle'
#     #
#     with open(pickle_path_symb_zero, "wb") as f:
#         pickle.dump(dict_obj_to_save, f)
#
